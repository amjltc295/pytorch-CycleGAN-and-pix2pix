...
class CycleGAN3DModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            parser.add_argument(
                '--lambda_A', type=float,
                default=10.0, help='weight for cycle loss (A -> B -> A)')
            parser.add_argument(
                '--lambda_B', type=float, default=10.0,
            ...
            parser.add_argument(
                ...
                help=(
                    'use identity mapping. Setting lambda_identity other than '
                    '0 has an effect of scaling the weight of the identity '
                    'mapping loss. For example, if the weight of the identity '
                    'loss should be 10 times smaller than the weight of the '
    ...
    def initialize(self, opt):
        ...
        # The program will call base_model.get_current_visuals
        ...
        # The program will call base_model.save_networks and
        ...
        if self.isTrain:
            ...
            self.netD_B = networks.define_D(
                opt.input_nc, opt.ndf, opt.which_model_netD,
                opt.n_layers_D, opt.norm, use_sigmoid,
                opt.init_type, opt.init_gain, self.gpu_ids
            )
    ...
    def set_input(self, input_images):
        ...
        self.real_A = input_images['A' if AtoB else 'B'].to(self.device)
        self.real_B = input_images['B' if AtoB else 'A'].to(self.device)
        self.image_paths = input_images['A_paths' if AtoB else 'B_paths']
